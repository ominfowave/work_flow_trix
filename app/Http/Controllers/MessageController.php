...
class MessageController extends Controller
{
    public function index(Request $request){
        ...
        if($request->ajax()){
            ...
            return response()->json(['html' => view('message.chat_list', compact('messages', 'receiver_id', 'sender_id'))->render() ]);
        }

        // $userList = Admin::select('users.*',   DB::raw('(SELECT COUNT(*) FROM messages WHERE messages.receiver_id = ? AND messages.is_read = "0", [$sender_id]) as read_count'))->get();
